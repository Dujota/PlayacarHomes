...
async function queryStaleRoutes(body: Pick<ParseBody['body'], '_type' | '_id' | 'date' | 'slug'>): Promise<StaleRoute[]> {
...
async function mergeWithMoreStories(client, slugs: string[]): Promise<string[]> {
  const moreStories = await client.fetch(groq`*[_type == "post"] | order(date desc, _updatedAt desc) [0...3].slug.current`);
