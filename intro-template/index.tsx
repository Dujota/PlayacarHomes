...
export default memo(function IntroTemplate() {
  ...
  const hasRepoEnvVars = process.env.NEXT_PUBLIC_VERCEL_GIT_PROVIDER && process.env.NEXT_PUBLIC_VERCEL_GIT_REPO_OWNER && process.env.NEXT_PUBLIC_VERCEL_GIT_REPO_SLUG;
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      setCreatePostURL(`${window.location.origin}/studio/intent/create/template=post;type=post/`);
    ...
    <div className='flex justify-center border border-gray-200 bg-gray-50'>
      <div className='mb-8 mt-20 grid max-w-screen-2xl grid-cols-1 gap-y-20 md:grid-cols-2 md:gap-x-16 md:gap-y-32 lg:gap-x-32 '>
        <div className='self-center'>
          <Image alt='An illustration of a browser window, a terminal window, the Sanity.io logo and the NextJS logo' src={cover} />
        ...
        <div className='mx-6 md:mx-0 md:mr-24'>
          ...
          <ol>
            <Box
              ...
              element={
                <div>
                  <div className='col-span-2 mb-2 mt-1 font-semibold'>Create content with Sanity Studio</div>
                  ...
                  <div className='mt-3'>
                    <Link className='inline-flex rounded bg-blue-500 px-4 py-2 text-white hover:bg-blue-800' href={createPostURL}>
                      Go to Sanity Studio
                    </Link>
                  </div>
                </div>
            ...
            <Box
              ...
              element={
                <div>
                  <div className='col-span-2 mb-2 mt-1 font-semibold'>Modify and deploy the project</div>
                  ...
                  {isLocalHost ? (
                    ...
                    <>
                      <div className='text-xs text-gray-700'>
                        ...
                        <a className='mx-1 underline hover:text-blue-800' href={repoURL} target='_blank' rel='noreferrer'>
                          {repoURL}
                        </a>
                      </div>

                      <div className='mt-3'>
                        <a className='inline-flex rounded bg-blue-500 px-4 py-2 text-white hover:bg-blue-800' href={repoURL} target='_blank' rel='noopener noreferrer'>
  ...
  return (
    <li className='mt-2 grid grid-flow-col grid-rows-1 place-content-start gap-3'>
      <div className='row-span-3 select-none'>
        <div className='relative flex h-6 w-6 select-none items-center justify-center rounded-full bg-gray-200 p-4 text-center'>{circleTitle}</div>
      </div>
      {element}
    </li>
  );
...
function BlueLink({ href, text }: { href: string; text: string }) {
  return (
    <a href={href} className='text-blue-500 underline hover:text-blue-800' target='_blank' rel='noreferrer'>
      {text}
    </a>
  );
}
